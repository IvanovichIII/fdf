...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/14 14:09:06 by igomez-s          #+#    #+#             */
/*   Updated: 2025/03/14 14:16:13 by ivan             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_down3(t_fdf *fdf, int puntos[4], int i, int j)
{
	if (fdf->modo == 2)
	{
		...
		puntos[2] = (((j + 1) - i) * fdf->zoom)
			/ (1 + (fdf->map[i + 1][j] * 0.005)) / 2;
		puntos[3] = (((j + 1) + i) * fdf->zoom - fdf->map[i + 1][j] * 2)
			/ (1 + (fdf->map[i + 1][j] * 0.005)) / 2;
...
void	draw_down(t_fdf *fdf, int puntos[4], int i, int j)
{
	...
	draw_down2(fdf, puntos, i, j);
	draw_down3(fdf, puntos, i, j);
	rotate(&puntos[0], &puntos[1], fdf->map[i][j], fdf);
	rotate(&puntos[2], &puntos[3], fdf->map[i + 1][j], fdf);
